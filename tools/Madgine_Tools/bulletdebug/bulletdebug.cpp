...
namespace Engine {
	namespace Tools {
		...
		{
			mManager = &static_cast<ClientImRoot&>(mRoot).window().app().getGlobalAPIComponent<Scene::SceneManager>().getComponent<Physics::PhysicsManager>();
		...
		void BulletDebug::renderSettings()
		{
		...
		void BulletDebug::update()
		{
			auto guard = mManager->sceneMgr().mutex().lock(AccessMode::READ);
